...
export function useNewsAdmin() {
  ...
  const createNews = useCallback(async (data: CreateNewsData): Promise<News | null> => {
    try {
  ...
  const updateNews = useCallback(async (id: string, data: UpdateNewsData): Promise<News | null> => {
    try {
